function []   = PlotDiffShadedFigure(pairedImagingData,fieldsStruct,analysisArgs)
colourMat = {'k','r','g','y','m','c','k','v','w','x','s','p','o'};
  xlims = [-2 (analysisArgs.lengthOfCurveAnalysis+5)];
userDefinedYLims = GetYlims(analysisArgs);

[indicesForFirstCyclingVariable, labelsForFirstCyclingVariable] = grp2idx([pairedImagingData.(fieldsStruct.fieldForCycling2)]);
[indicesForSecondCyclingVariable, labelsForSecondCyclingVariable] = grp2idx([pairedImagingData.(fieldsStruct.fieldForCycling)]);
[indicesForVariableForComparison, labelsForVariableForComparison] = grp2idx([pairedImagingData.(fieldsStruct.fieldToCompare)]);
...
for ii = 1: length(labelsForFirstCyclingVariable)
    ...
    for jj = 1: length(labelsForSecondCyclingVariable)
        ...
        shadedPlotElement = 1;clear h_shadedPlot
            ...
            indicesForThisGroup = indicesForFirstCyclingVariable==ii & indicesForSecondCyclingVariable==jj;
            plotValues =  horzcat(pairedImagingData(indicesForThisGroup).outputPlotValsChange)';
            if sum(indicesForThisGroup)>2
            % h_shadedPlot(shadedPlotElement) = shadedErrorBar(pairedImagingData(1).tCroppedPlot,plotValues,{@median,@mad},colourMat{shadedPlotElement},0);
            h_shadedPlot(shadedPlotElement) = shadedErrorBar(pairedImagingData(1).tCroppedPlot,plotValues,{@mean,@GetSEM},colourMat{shadedPlotElement},0);
            hold on
            shadedPlotElement = shadedPlotElement+1;
            labelsForLegend{strcmp(labelsForLegend,labelsForVariableForComparison)} = [' n = ' num2str(sum(indicesForThisGroup))];
            else
                disp([labelsForFirstCyclingVariable{ii} ', ' labelsForSecondCyclingVariable{jj} ', '   labelsForVariableForComparison ' has an insufficient number of datapoints to plot'])
                labelsForLegend(strcmp(labelsForLegend,labelsForVariableForComparison)) = [];
            end
...
pos = [448 208 344 406],set(gcf,'Position',pos)
